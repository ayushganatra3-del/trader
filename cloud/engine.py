...
def session_date(value):
    """Require a canonical date before using it as a storage path component."""
    if not isinstance(value, str):
        raise ValueError("Session date must be YYYY-MM-DD")
    try:
        parsed = dt.date.fromisoformat(value)
    except ValueError as error:
        raise ValueError("Session date must be YYYY-MM-DD") from error
...
def boundary(day, hour, minute=0):
    return dt.datetime.combine(session_date(day), dt.time(hour, minute), LONDON).astimezone(UTC)
...
def parse_snapshot(symbol, payload, observed_at, session_day, source="injected offline fixture", allow_stale_reconciliation=False):
    """Validate one read-only Yahoo chart observation; prices returned in GBP."""
    session_date(session_day)
    if symbol not in SYMBOLS:
        raise ValueError("Unapproved symbol")
    if london_day(observed_at) != session_day:
    ...
    if market_time is not None:
        ...
        if london_day(dt.datetime.fromtimestamp(market_time, UTC)) != session_day:
    ...
    for index, raw_time in enumerate(timestamps):
        ...
        if london_day(bar_date) != session_day:
            continue
        if not boundary(session_day, 8).timestamp() <= start < boundary(session_day, 16, 30).timestamp():
            continue
...
def initial_state(now, session_day):
    session_date(session_day)
    return {"version": 1, "session_day": session_day, "created_at": stamp(now),
            ...
            "stop_requested_at": None, "last_errors": {},
            "rules": {"symbols": list(SYMBOLS), "buy_budget_gbp": BUY_BUDGET,
                      ...
                      "timezone": "Europe/London", "no_entries_after_local": "15:45",
                      "exit_queue_local": "16:15", "regular_open_local": "08:00",
                      "regular_close_local": "16:30", "final_reconciliation_local": "17:00"}}
...
def decide(state, symbol, side, now, reason, quote=None):
    if any(intent["symbol"] == symbol and intent["side"] == side for intent in pending(state)):
        return
    intent = {"intent_id": f"{state['session_day']}-{state['next_id']:04d}", "symbol": symbol,
...
def recognise_fills(state, quotes, now):
    ...
    for intent in pending(state):
        ...
        candidates = [bar for bar in quote["bars"] if bar["start"] >= decision_time and bar["start"] < boundary(session_day, 16, 30).timestamp() and (intent["side"] == "sell" or bar["start"] < boundary(session_day, 15, 45).timestamp())]
        if not candidates:
            continue
        bar = candidates[0]
        raw_price = bar["open"]
...
def step(state, quotes, errors, now):
    ...
    session_day = state["session_day"]
    session_date(session_day)
    if state["status"] in ("complete", "incomplete") or london_day(now) != session_day:
        return {"status": state["status"], "new_fills": [], "alerts": ["Session is closed; no actions taken"], **totals(state)}
    if now < boundary(session_day, 8):
        return {"status": "waiting_for_open", "new_fills": [], "alerts": [], **totals(state)}
    ...
    if now >= boundary(session_day, 15, 45):
        cancel_buys(state, now, "No further entry fills at or after 15:45 Europe/London")
    ...
    final = now >= boundary(session_day, 17) or (state.get("stop_requested_at") and not state["positions"])
    if not final:
        for symbol, position in list(state["positions"].items()):
            ...
            elif now >= boundary(session_day, 16, 15):
                reason = "Scheduled 16:15 Europe/London close"
            elif quote:
        ...
        if now < boundary(session_day, 15, 45) and not state["halted"]:
...
def write_journal(state, path):
    ...
    lines = [f"# £100 virtual stock-market session — {state['session_day']}", "",
    ...
    lines += ["", "## Fixed rules and accounting assumptions", "",
              f"- Session: {state['session_day']} only. London regular hours 08:00–16:30 Europe/London, adjusted for daylight saving. Each run observes once; missed checks do not create historical decisions.",
              ...
              "- No new buy decisions at or after 15:45 Europe/London. Scheduled exits are queued on the first check at or after 16:15 Europe/London. No buy fill may have an effective time at or after that cutoff. Pending buys with no eligible earlier fill are cancelled at the cutoff after reconciliation.",
              "- Fills require a bar starting before the 16:30 Europe/London regular close. At or after 17:00 Europe/London, one final reconciliation finalises the session. Any unsold exposure stays visible as incomplete; no closing sale or final profit is invented.",
...
def save_state(state, folder):
    ...
    if state_path.exists():
        ...
        if previous.get("session_day") != state.get("session_day"):
            raise ValueError("Session date changed; refusing to overwrite its record")
...
def fetch_snapshot(symbol, folder, session_day):
    if symbol not in SYMBOLS:
        raise ValueError("Unapproved symbol")
    session_date(session_day)
    ...
    return parse_snapshot(symbol, payload, observed_at, session_day, source=f"sessions/{session_day}/snapshots/{filename}", allow_stale_reconciliation=True)


def expire_missed_session(state, now):
    """Administrative finalisation only; never fetch, fill or decide after today."""
    if london_day(now) <= state["session_day"] or state["status"] in ("complete", "incomplete"):
        return
...
def request_stop(state, now):
    """Stop is a new present-time instruction, never a historical fill."""
    if state["status"] in ("complete", "incomplete"):
        return
    if state.get("last_check_at") and now < instant(state["last_check_at"]):
        raise ValueError("Wall clock moved backwards; refusing to stop in the past")
    if london_day(now) > state["session_day"]:
        expire_missed_session(state, now)
        return
    if state.get("stop_requested_at") and now < boundary(state["session_day"], 17):
        return
    if not state.get("stop_requested_at"):
        state.update(halted=True, halt_reason="User requested stop", stop_requested_at=stamp(now))
        add_event(state, "stop_requested", now, reason=state["halt_reason"])
    cancel_buys(state, now, state["halt_reason"])
    if now < boundary(state["session_day"], 17):
        for symbol in sorted(state["positions"]):
            decide(state, symbol, "sell", now, state["halt_reason"])
    if not state["positions"] or now >= boundary(state["session_day"], 17):
        for intent in pending(state, "sell"):
            intent.update(status="unresolved", unresolved_at=stamp(now))
            add_event(state, "unresolved", now, intent_id=intent["intent_id"], symbol=intent["symbol"],
                      reason="Session deadline passed; no closing fill fabricated")
        state.update(status="incomplete" if state["positions"] else "complete", finalised_at=stamp(now))
        add_event(state, "finalised", now, status=state["status"], reason="User requested stop",
                  open_symbols=sorted(state["positions"]), **totals(state))
    assert_accounting(state)
