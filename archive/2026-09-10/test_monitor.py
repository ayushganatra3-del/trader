...
def quote(symbol="ISF.L", last_start="07:25:00", observed="07:31:00", **kwargs):
    ...
    return m.parse_snapshot(symbol, fixture(symbol, last_start, observed, **kwargs), at(observed), source="offline fixture", allow_stale_reconciliation=allow_stale)


def seed_position():
    state = m.initial_state(at("07:31:00"))
...
class DataValidationTests(unittest.TestCase):
    ...
    def test_irregular_yahoo_latest_trade_row_ignored(self):
        ...
        q = m.parse_snapshot("ISF.L", payload, at("07:51:00"))
    ...
    def test_provider_time_future_or_wrong_day_rejected(self):
        ...
        with self.assertRaisesRegex(ValueError, "today"):
            m.parse_snapshot("ISF.L", payload, at("07:31:00"))
    ...
    def test_missing_bars_cannot_masquerade_as_20_minutes(self):
        ...
        for values in result["indicators"]["quote"][0].values():
            del values[-3]
        self.assertIsNone(m.parse_snapshot("ISF.L", payload, at("07:41:00"))["momentum_20m"])
...
class ExecutionTests(unittest.TestCase):
    ...
    def test_no_buy_effective_at_or_after_entry_cutoff(self):
        state = m.initial_state(at("14:40:01"))
...
class PersistenceTests(unittest.TestCase):
    ...
    def test_finished_run_never_fetches_market_data(self):
        with tempfile.TemporaryDirectory() as folder:
            state = seed_position()
            m.step(state, {}, {}, at("16:00:01"))
            m.save_state(state, folder)
            with patch.object(m, "fetch_snapshot", side_effect=AssertionError("Network forbidden")):
                self.assertEqual(m.run(folder)["status"], "incomplete")
