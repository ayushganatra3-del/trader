...
| Machine learning | **Agent (ML meta-label)**: Lopez de Prado's meta-labeling. The hourly strategies propose trades, which are the signals that survive costs. A gradient-boosted tree model, retrained every day on past trades only, predicts which proposals will win and skips the rest. It sizes the trades it takes at half-Kelly, with at most 2% of equity at risk and 25% per position. Tests check that it never uses future data and finds nothing to trade on a pure random walk. |
